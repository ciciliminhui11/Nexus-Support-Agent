...
- [ ] T003 [P] 创建 `backend/.env.example` 占位模板，包含四个密钥/模型变量键名（值留空占位）：`DEEPSEEK_API_KEY=`、`DEEPSEEK_BASE_URL=`、`DEEPSEEK_SMALL_MODEL=DeepSeek-R1-0528-Qwen3-8B`、`DEEPSEEK_LARGE_MODEL=`。真实密钥由用户后续填写、不提交仓库（FR-014）。
...
- [ ] T005 [P] 在 `backend/app/core/config.py` 追加 intent 配置节：定义 `IntentSettings`（pydantic-settings），从 `.env` 读取 `DEEPSEEK_API_KEY`、`DEEPSEEK_BASE_URL`、`DEEPSEEK_SMALL_MODEL`（默认 `DeepSeek-R1-0528-Qwen3-8B`）、`DEEPSEEK_LARGE_MODEL`；从 `system_config` 表/配置读取阈值与参数：`intent_high_threshold`（默认 0.9）、`intent_low_threshold`（默认 0.6）、`intent_clarify_retry`（默认 1）、`intent_reverse_calibrate`（默认 true）、`intent_model_self_check`（默认 false），以及词库/句式模板/负样本文件路径（指向 `backend/config/intent_keywords.yaml`、`intent_patterns.yaml`、`intent_negative_samples.yaml`）。
